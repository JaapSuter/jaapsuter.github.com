    ...
    for child in element.childNodes:
      ...
      if child.nodeType == 1:
        if preserveWhitespace:
          ...
          outParts.extend([N, serializeXML(child, options, indent + 1, preserveWhitespace)])
          onNewLine = True
      # text node
...
def scourString(in_string, options=None):
  ...
  for i in xrange(attrList.length):
    ...
    name = attr.nodeName
    val = attr.nodeValue
    if options.root_svg_minimal_for_web:
      if name != "height" and name != "width" and name != "xmlns":
        redundantPrefixes.append(name[6:])
        xmlnsDeclsToRemove.append(name)
    elif name[0:6] == 'xmlns:' and val == 'http://www.w3.org/2000/svg':
      redundantPrefixes.append(name[6:])
      xmlnsDeclsToRemove.append(name)
...
_options_parser.add_option("--keep-only-width-and-height-on-root",
  action="store_true", dest="root_svg_minimal_for_web", default=True,
  help="remove all but what's necessary for the web")
...
_options_parser.add_option("-p", "--set-precision",
  action="store", type=int, dest="digits", default=3,
